...
TopicForge grounds the agent on the *runtime* robot stack; DatasetForge
grounds it on the *training data* that produced the perception models
running on that stack. Read-only by design — no editing, no relabeling.
